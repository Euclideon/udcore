...
#if UDASSERT_ON
#  define UDASSERT(condition, ...)        \
    do                                    \
    {                                     \
      bool testCondition = !!(condition); \
      if (!testCondition)                 \
      {                                   \
        udDebugPrintf(__VA_ARGS__);       \
        DebugBreak();                     \
        udDebugPrintf("\n");              \
      }                                   \
    } while (0)
...
#  define UDASSERT(condition, ...) \
    do                             \
    {                              \
    } while (0) // TODO: Make platform-specific __assume(condition)
#  define IF_UDASSERT(x)
#endif // UDASSERT_ON

#if UDRELASSERT_ON
#  define UDRELASSERT(condition, ...)     \
    do                                    \
    {                                     \
      bool testCondition = !!(condition); \
      if (!testCondition)                 \
...
#  define PRINT_ERROR_STRING(...)           \
    do                                      \
    {                                       \
      udDebugPrintf("%s : ", __FUNCTION__); \
      udDebugPrintf(__VA_ARGS__);           \
      __breakOnErrorString();               \
    } while (false)
