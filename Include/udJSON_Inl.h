...
inline udJSON::udJSON(int64_t v)
{
  type = T_Int64;
  u.i64Val = v;
  dPrec = 0;
}
inline udJSON::udJSON(double v)
{
  type = T_Double;
  u.dVal = v;
  dPrec = 0;
}
inline void udJSON::Clear()
{
  type = T_Void;
  u.i64Val = 0;
  dPrec = 0;
} // Clear the value without freeing
inline udJSON::~udJSON() { Destroy(); }

// Set the value
inline void udJSON::SetVoid() { Destroy(); }
inline void udJSON::Set(bool v)
{
  Destroy();
  type = T_Bool;
  u.bVal = v;
}
inline void udJSON::Set(int64_t v)
{
  Destroy();
  type = T_Int64;
  u.i64Val = v;
}
inline void udJSON::Set(double v)
{
  Destroy();
  type = T_Double;
  u.dVal = v;
...
inline size_t udJSON::ArrayLength() const { return (type == T_Array) ? u.pArray->length : (type == T_Object) ? 1
                                                                                                             : 0; }
...
inline udResult udJSON::ExtractAndVoid(const char **ppStr)
{
  if (!ppStr)
