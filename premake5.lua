...
solution "udCore"
	...
	elseif os.target() == "emscripten" then
		...
		buildoptions { "-s USE_PTHREADS=1", "-s ABORTING_MALLOC=0", "-s FETCH=1", "-s PROXY_TO_PTHREAD=1", "-s WASM=1", "-s ALLOW_MEMORY_GROWTH=1" }
		linkoptions  { "-s USE_PTHREADS=1", "-s ABORTING_MALLOC=0", "-s FETCH=1", "-s PROXY_TO_PTHREAD=1", "-s WASM=1", "-s ALLOW_MEMORY_GROWTH=1" }
		targetextension ".bc"
		filter { "kind:*App" }
			targetextension ".js"
		filter { "files:**.cpp" }
