...
solution "udCore"
	...
	elseif os.target() == "emscripten" then
		...
		buildoptions { "-s ALLOW_TABLE_GROWTH", "-s USE_PTHREADS=1", "-s PTHREAD_POOL_SIZE=8", "-s ABORTING_MALLOC=0", "-s FETCH=1", "-s PROXY_TO_PTHREAD=1", "-s WASM=1", "-s TOTAL_MEMORY=1073741824" }
		linkoptions  { "-s ALLOW_TABLE_GROWTH", "-s USE_PTHREADS=1", "-s PTHREAD_POOL_SIZE=8", "-s ABORTING_MALLOC=0", "-s FETCH=1", "-s PROXY_TO_PTHREAD=1", "-s WASM=1", "-s TOTAL_MEMORY=1073741824" }
		targetextension ".bc"
		filter { "kind:*App" }
			targetextension ".js"
		filter { "files:**.cpp" }
