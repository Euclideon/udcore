...
TEST(udCompressionTests, Basic)
{
  for (int i = 1; i < udCT_Count; ++i) //Skip udCT_None
  {
    ...
    EXPECT_NE(0, memcmp(input, pDeflated, min(UDARRAYSIZE(input), deflatedSize)));
    ...
    EXPECT_EQ(0, memcmp(input, inflated, min(UDARRAYSIZE(input), inflatedSize)));
...
TEST(udCompressionTests, Complex)
{
  for (int i = 1; i < udCT_Count; ++i) //Skip udCT_None
  {
    ...
    EXPECT_NE(0, memcmp(input, deflated, min(UDARRAYSIZE(input), deflatedSize)));
...
TEST(udCompressionTests, Zip)
{
  ...
  pDOC = udAllocType(char, max(doc1Len, max(doc2Len, doc3Len)) + 1, udAF_None); // Extra byte for nul terminator
