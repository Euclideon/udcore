...
{
  BYTE buf_iv[AES_BLOCK_SIZE], buf_out[AES_BLOCK_SIZE];
	int blocks, idx;
	...
	for (idx = 0; idx < blocks; idx++) {
		aes_decrypt(&in[idx * AES_BLOCK_SIZE], buf_out, key, keysize);
		xor_buf(buf_iv, buf_out, AES_BLOCK_SIZE);
		memcpy(buf_iv, &in[idx * AES_BLOCK_SIZE], AES_BLOCK_SIZE);
		memcpy(&out[idx * AES_BLOCK_SIZE], buf_out, AES_BLOCK_SIZE);
	}
